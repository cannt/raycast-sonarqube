...
jest.mock("@raycast/api", () => {
  ...
  const ActionPanel = ({ children, ...props }: any) => (
    <div data-testid="action-panel" {...filterRaycastProps(props)}>
      {children}
    </div>
  );
  ActionPanel.Section = ({ children, ...props }: any) => (
    <div data-testid="action-section" {...filterRaycastProps(props)}>
      {children}
    </div>
  );
  ...
  const Action = Object.assign(
    ...
    },
    {
      ...
      SubmitForm: ({ title, onSubmit, ...props }: any) => (
        <button type="submit" onClick={onSubmit} data-testid={`submit-action-${title}`} {...props}>
          {title}
        </button>
      ),
    },
  ...
  return {
    ...
    Form: Object.assign(
      ...
      {
        TextField: ({ id, title, placeholder, value, onChange, error, info, ...props }: any) => (
        ...
        <div data-testid="navigation-stack">
          ...
          <div data-testid="stack-previous" style={{ display: "none" }}>
            {stack.slice(0, -1).map((item, i) => (
              <div key={`stack-${i}`} data-testid={`stack-item-${i}`}>
                {item}
              </div>
            ))}
          </div>
          ...
          {/* Current view */}
          <div data-testid="stack-current">{stack[stack.length - 1]}</div>
        </div>
      );
    }
    return children;
...
describe("runSonarAnalysis UI", () => {
  ...
  it("can add a new project", async () => {
    loadProjects.mockResolvedValue([]);
    const { findByText, getByPlaceholderText, findByTestId, container } = render(
      <NavigationTestWrapper>
        <Command />
      </NavigationTestWrapper>,
    );
  ...
  it("validates project form input", async () => {
    ...
    const { findByText, getByPlaceholderText, queryByText, findByTestId } = render(
      <NavigationTestWrapper>
        <Command />
      </NavigationTestWrapper>,
    );
  ...
  it("uses custom SonarQube app path when enabled in preferences", async () => {
    ...
    await waitFor(() => {
      ...
      const customPathCommand = commands.find((cmd: string) => cmd.includes("/Applications/SonarQube.app"));
