...
describe("i18n utils", () => {
  ...
  describe("t function", () => {
    ...
    it("returns SonarQube status detection translations for English", () => {
      ...
      expect(i18n.t("commands.startSonarQube.initializing")).toBe(
        "SonarQube might be initializing. Checking again with longer timeout...",
      );
    });

    it("returns SonarQube status detection translations for Spanish", () => {
      mockGetPreferenceValues.mockReturnValue({ language: "es" });
      expect(i18n.t("commands.startSonarQube.pleaseWait")).toBe(
        "SonarQube está iniciándose, por favor espera un momento",
      );
      expect(i18n.t("commands.startSonarQube.checkingStatus")).toBe("Comprobando el estado de SonarQube...");
      expect(i18n.t("commands.startSonarQube.initializing")).toBe(
        "SonarQube podría estar inicializándose. Comprobando de nuevo con mayor tiempo de espera...",
      );
    });
