...
jest.mock("../index", () => {
  ...
  return {
    ...
    // Fix generateId to return a string of exactly 9 characters that passes the regex test
    generateId: jest.fn().mockReturnValue("test12345"),
    // Explicitly mock the storage functions to track the calls
    loadProjects: jest.fn().mockImplementation(async () => {
      const val = await LocalStorage.getItem("sonarqubeProjectsList");
      if (!val) return [];
      try {
...
import { Project, generateId, saveProjects, loadProjects, isSonarQubeRunning } from "../index";
