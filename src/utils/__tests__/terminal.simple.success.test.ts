...
const customTerminalModule = {
  ...
  runCommand: async (command: string, successMessage: string, failureMessage: string, options?: CommandOptions) => {
    // Show initial toast
    ...
    try {
      ...
      toast.title = failureMessage;
      toast.message =
        error instanceof Error ? customTerminalModule.getUserFriendlyErrorMessage(error.message) : "Unknown error";
