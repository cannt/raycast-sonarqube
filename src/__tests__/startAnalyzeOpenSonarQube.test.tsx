...
jest.mock("@raycast/api", () => ({
  ...
  List: {
    Item: jest.fn(({ title, actions }) => (
      <div data-testid={`list-item-${title}`}>{title}{actions}</div>
    )),
...
jest.mock("../components/ProjectsList", () => ({
  ProjectsList: jest.fn(({ projects, isLoading, onStartAnalyze }: { projects: Array<{id: string, name: string, path: string}>, isLoading: boolean, onStartAnalyze: (path: string, name: string) => void }) => {
    if (isLoading) {
      return <div data-testid="empty-view"><div>Loading...</div></div>;
    }
    if (projects.length === 0) {
      return <div data-testid="empty-view"><div>No projects found</div></div>;
    }
    return (
      <div>
